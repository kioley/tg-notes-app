...
export async function updateFolderForUser(
  ...
  `
    ...
    .run(name.trim(), color, folderId, userId);
...
export async function deleteFoldersForUser(
  folderIds: ID[],
  userId: number
): Promise<{ deletedCount: number; totalCount: number }> {
  console.log(
    `📁 Deleting folders [${folderIds.join(", ")}] for user ${userId}`
  );
  ...
  console.log(
    `📦 Deleted ${itemsResult.changes} items in folders [${folderIds.join(
      ", "
    )}]`
  );

  // Затем удаляем сами папки
  const foldersResult = db
    .query(
      `
    UPDATE folders 
    SET deleted_at = CURRENT_TIMESTAMP 
