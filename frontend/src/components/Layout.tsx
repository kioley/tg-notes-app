...
import HighlightButtons from "./buttons/HighlightButtons";

function Layout() {
  const { currentView, showCreateForm, isHighlightMode } = useAppStore();
  ...
  const renderButtons = () => {
    switch (currentView) {
      case "folders":
        if (isHighlightMode) return <HighlightButtons />;
        if (showCreateForm) return <CreateFolderButtons />;
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-50">
      ...
      {buttons && (
        <div className="p-4 bg-white border-t flex gap-2">{buttons}</div>
      )}
    </div>
  );
}
